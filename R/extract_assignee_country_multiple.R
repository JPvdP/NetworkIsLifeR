#' Geolocalisation with multiple countries per organisation
#'
...
#'
#' @details
#' The function filters out rows where either \code{name} or \code{residence} is NA.
...
#' #   Global Corp USA       3               60.0        5             New York 66.7
#' #   Global Corp Germany   2               40.0        5             Berlin   100.0
#' #   Local Inc   Canada    1               100.0       1             Toronto  100.0
#'
#' @seealso \code{\link{assign_most_frequent}} for assigning a single country
...
extract_assignee_country_multiple <- function(df) {
  ...
  df %>%
    dplyr::filter(!is.na(name) & !is.na(residence)) %>%
    dplyr::group_by(name, residence) %>%
    dplyr::summarise(
      address = {
    ...
    dplyr::ungroup() %>%
    dplyr::select(name, residence, country_records, country_pct, total_records,
                  address, address_pct) %>%
    dplyr::arrange(name, dplyr::desc(country_records))
}
