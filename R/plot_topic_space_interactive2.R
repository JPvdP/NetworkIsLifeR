#' Interactive 2D Topic Map with Plotly2
#'
...
plot_topic_space_interactive2 <- function(
    ...
    y_col = "dim_2",
    cluster_col = "cluster",
    topic_labels = NULL,  # labels per topic (data.frame or named character vector)
    show_ids = TRUE       # prepend "Topic k: " in the label
) {
  ...
  label_df <- NULL
  labels_from_table <- FALSE

  if (!is.null(topic_labels)) {
    if (is.data.frame(topic_labels)) {
      labels_from_table <- TRUE
      tmp <- topic_labels

      # Accept cluster/topic and label/Label
      ...
      label_df <- tmp %>%
        dplyr::mutate(
          cluster = as.integer(.data$cluster),
  ...
  if (!is.null(label_df)) {
    df <- df %>%
      dplyr::left_join(label_df, by = dplyr::join_by(cluster_num == cluster)) %>%
      dplyr::mutate(
        topic_label = dplyr::case_when(
          cluster_num == 0 ~ "Noise",
          !is.na(label) & show_ids  ~ paste0("Topic ", cluster_num, ": ", label),
          !is.na(label) & !show_ids ~ label,
          TRUE ~ paste0("Topic ", cluster_num)
        )
      )
  } else {
    df <- df %>%
      dplyr::mutate(
        topic_label = dplyr::if_else(cluster_num == 0, "Noise", paste0("Topic ", cluster_num))
      )
  }
  ...
  if (is.null(text)) {
    ...
    if (length(text) != nrow(df)) stop("If provided, `text` must have the same length as nrow(doc_coordinates).")
    df$tooltip <- paste0(
      "Doc: ", df$doc_id, "<br>",
      "Topic: ", df$topic_label, "<br>",
      "Text: ", text
  ...
  # --- 7. Add topic labels at centroids (conditionally) ---
  if (add_on_map_labels) {
    p <- plotly::add_text(
      p,
      data = centroids,
