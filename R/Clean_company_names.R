...
.rule_matches <- function(x, type, tokens, pattern, ignore_case = TRUE) {
  # --- pattern branch (only if it's truly usable) ---------------------------
  pat <- trimws(as.character(pattern)[1])
  has_pat <- !is.na(pat) && nzchar(pat) && toupper(pat) != "NA"
  if (has_pat) {
    return(grepl(pat, x, ignore.case = ignore_case, perl = TRUE))
  }

  # --- tokens branch --------------------------------------------------------
  toks <- unlist(tokens, use.names = FALSE)
  toks <- trimws(as.character(toks))
  toks <- toks[!is.na(toks) & nzchar(toks)]
  if (!length(toks)) return(rep(FALSE, length(x)))

  type <- toupper(trimws(as.character(type)[1]))

  if (type == "ALL") {
    return(Reduce(`&`, lapply(toks, function(t) grepl(t, x, ignore.case = ignore_case, perl = TRUE))))
  }
  if (type == "ANY") {
    return(Reduce(`|`, lapply(toks, function(t) grepl(t, x, ignore.case = ignore_case, perl = TRUE))))
  }
